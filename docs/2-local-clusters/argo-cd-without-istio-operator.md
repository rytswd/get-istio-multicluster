...
> 📍 **WARNING** 📍  
> This step is NOT a part of GitOps, because there is no easy way to have it in declarative manner, due to the cluster IP associated with `istiocoredns` is only confirmed once the Service is created. However, if you are trying to set up GitOps setup similar to this repository, you can follow the below instructions, and then place `coredns-configmap.yaml` file as a part of GitOps.  
